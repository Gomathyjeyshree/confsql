...
export default function ConferenceByMonth() {
  ...
  useEffect(() => {
    ...
    // Send the month param to backend
    fetch(`http://localhost:5000/api/eventsByMonth?month=${encodeURIComponent(month)}`)
      .then(res => {
        if (!res.ok) throw new Error("Failed to fetch conferences");
        return res.json();
      })
      .then(data => setConferences(data))
      .catch(err => setError("Unable to load conferences."))
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          {conferences.length > 0 ? (
            conferences.map((conf) => (
              ...
              >
                <h3 className="font-bold text-purple-700">{conf.event_name}</h3>
                <p className="text-gray-600 text-sm">
                  {conf.sdate ? new Date(conf.sdate).toLocaleDateString("en-GB") : "-"} - {conf.country || "-"}
                </p>
