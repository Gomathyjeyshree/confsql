...
export default function ListingByTopics() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/eventsByTopic/${topic}`)
...
const stripHtml = (html) => {
  ...
  if (!events.length) return <p className="text-center py-12">No events found for {topic}.</p>;
        ...
        <tbody>
          {events.map((event, idx) => (
            <tr key={idx}>
              ...
              <td className="px-6 py-4">{new Date(event.sdate).toLocaleDateString("en-GB")}</td>
