...
export default function ListingByContinentSubtopic() {
  const { continent, subtopic } = useParams();
  ...
  useEffect(() => {
    if (!continent || !subtopic) return;
    setLoading(true);

    axios.get(`http://localhost:5000/api/eventsByContinentSubtopic/${continent}/${subtopic}?page=${page}&limit=10`)
      .then(res => {
  ...
  if (!events.length) return <p>No events found in {continent} for {subtopic}.</p>;

  return (
    <div className="bg-gray-50 min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <h2 className="text-2xl font-bold text-center py-4">
          Events in {continent} – {subtopic}
        </h2>
